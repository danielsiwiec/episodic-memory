...
#### 3. Limit Flag (`-l` / `--limit`)

Limit the number of conversations to process in a single run. Useful for batch processing large backlogs without risking rate limits or long-running operations.

**Usage:**
```bash
# Index only the first 100 conversations
episodic-memory index --limit 100

# Combine with other flags
episodic-memory index -d 30 -l 50 -c 4
```

**Files changed:**
- `src/index-cli.ts` - CLI argument parsing
- `cli/index-conversations.js` - Help text
