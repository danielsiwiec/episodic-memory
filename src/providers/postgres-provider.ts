 ...
 */
export class PostgresProvider implements DatabaseProviderInterface {
  ...
  private async migrateSchema(client: pg.PoolClient): Promise<void> {
  ...
  async getFileLastIndexed(archivePath: string): Promise<number | null> {
    ...
    const result = await this.pool.query(
      'SELECT MAX(last_indexed) as "lastIndexed" FROM exchanges WHERE archive_path = $1',
      [archivePath]
    );
  ...
  async hasExchangesForArchive(archivePath: string): Promise<boolean> {
    ...
    const result = await this.pool.query(
      'SELECT COUNT(*) as count FROM exchanges WHERE archive_path = $1',
      [archivePath]
    );
  ...
  async searchByVector(
    ...
    const params: unknown[] = [this.vectorToString(embedding), limit];
    const conditions: string[] = ['e.embedding IS NOT NULL'];
    let paramIndex = 3;
  ...
  async searchByText(
    ...
    const params: unknown[] = [likePattern, likePattern, limit];
    const conditions: string[] = ['(e.user_message ILIKE $1 OR e.assistant_message ILIKE $2)'];
    let paramIndex = 4;
  ...
  async getStats(): Promise<DatabaseStats> {
    ...
    // Total conversations
    const totalConversationsResult = await this.pool.query(
    ...
    // All archive paths
    const archivePathsResult = await this.pool.query(
   ...
   */
  async getSyncedFile(sourcePath: string): Promise<{ mtimeMs: number; sizeBytes: number } | null> {
    if (!this.pool) throw new Error('Database not initialized');

    const result = await this.pool.query(
      'SELECT mtime_ms, size_bytes FROM synced_files WHERE source_path = $1',
      [sourcePath]
    );
   ...
   */
  async setSyncedFile(sourcePath: string, mtimeMs: number, sizeBytes: number): Promise<void> {
    if (!this.pool) throw new Error('Database not initialized');
    ...
    await this.pool.query(`
      ...
      ON CONFLICT (source_path) DO UPDATE SET
        ...
        last_synced = NOW()
    `, [sourcePath, mtimeMs, sizeBytes]);
  }

  /**
   * Check if a file needs to be synced (new or modified)
   */
  async needsSync(sourcePath: string, mtimeMs: number, sizeBytes: number): Promise<boolean> {
   ...
   */
  async setSummary(sessionId: string, project: string, summary: string): Promise<void> {
    if (!this.pool) throw new Error('Database not initialized');
   ...
   */
  async getSessionsNeedingSummaries(limit: number = 10): Promise<Array<{ sessionId: string; project: string }>> {
    if (!this.pool) throw new Error('Database not initialized');
