...
export class SqliteProvider {
    ...
    migrateSchema() {
        ...
        const columns = this.db.prepare(`SELECT name FROM pragma_table_info('exchanges')`).all();
    ...
    async hasExchangesForArchive(archivePath) {
        ...
        const result = stmt.get(archivePath);
    ...
    `);
        return stmt.all(Buffer.from(new Float32Array(embedding).buffer), limit);
    }
    ...
    async getStats() {
        if (!this.db)
            throw new Error('Database not initialized');
        // Total conversations
        const totalConversations = this.db.prepare('SELECT COUNT(DISTINCT archive_path) as count FROM exchanges').get();
        // Total exchanges
        const totalExchanges = this.db.prepare('SELECT COUNT(*) as count FROM exchanges').get();
        // Date range
        const dateRange = this.db.prepare('SELECT MIN(timestamp) as earliest, MAX(timestamp) as latest FROM exchanges').get();
        // Project count
        const projectCount = this.db.prepare('SELECT COUNT(DISTINCT project) as count FROM exchanges').get();
        // Top 10 projects
    ...
    `).all();
        // All archive paths
        const archivePathsResult = this.db.prepare('SELECT DISTINCT archive_path FROM exchanges').all();
        return {
